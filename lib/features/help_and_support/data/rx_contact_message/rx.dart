...
final class ContactMessageRx extends RxResponseInt<Map> {
  ContactMessageRx({required super.empty, required super.dataFetcher});

  ValueStream get getCartStream => dataFetcher.stream;
  final api = ContactMessageApi.instance;

  Future<bool> sendMessage({required String message}) async {
    try {
      final data = await api.sendMessage(message: message);
