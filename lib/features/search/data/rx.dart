...
import 'package:llr/features/search/data/api.dart';
...
final class FestivalSearchRx extends RxResponseInt<SearchFestivalResponse> {
  FestivalSearchRx({required super.empty, required super.dataFetcher});

  ValueStream get getProfileStream => dataFetcher.stream;
  final api = FestivalSearchApi.instance;

  Future<bool> searchFestival({String? search}) async {
    try {
      final data = await api.searchFestival(search: search!);
